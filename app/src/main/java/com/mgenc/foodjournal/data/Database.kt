...
interface FoodEntryDao {
    @Query("SELECT * FROM food_entries WHERE epochDay = :epochDay ORDER BY mealType, createdAt")
    fun getByDate(epochDay: Long): Flow<List<FoodEntry>>

    @Query("SELECT * FROM food_entries ORDER BY epochDay DESC, mealType, createdAt")
